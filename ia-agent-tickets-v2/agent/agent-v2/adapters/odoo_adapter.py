...
class OdooAdapter(ITicketPort):
    ...
    def approve_ticket(self, ticket_id: int, user_id: int) -> dict:
        """
        Approves a pending ticket in Odoo.

        Sets approval_status='approved' and approval_date to now.
        Both fields are from ITS_Helpdesk_custom — approval_date is a Datetime
        field that Odoo accepts in '%Y-%m-%d %H:%M:%S' format via JSON-RPC.
        """
    ...
    def reject_ticket(self, ticket_id: int, reason: str, user_id: int) -> dict:
        """
        Rejects a pending ticket in Odoo.

        Sets approval_status='rejected' and stores the reason in approval_comment.
        NOTE: Odoo uses approval_comment (Text field) — NOT rejection_reason,
        which does not exist in ITS_Helpdesk_custom. The express_adapter dev
        field 'rejection_reason' maps to this field in production.
        """
        self._call_kw(
            "helpdesk.ticket.base", "write",
            [[ticket_id], {
